...
if st.session_state["role"] == "admin":
    with st.expander("Admin Panel", expanded=False):
        ...
        if users:
            for user in users:
                user_id, uname, _, _, user_role = user
...
# --- FILE UPLOAD ---
available_datasets = ["Iris", "Wine", "Diabetes", "Breast Cancer", "California Housing",
...
if df is not None and not df.empty:
    ...
    try:
            ...
            if split_mode == "Manual":
                ...
                if st.button("Split Data"):
                    if use_kfold:
                        ...
                        st.session_state.train_set, st.session_state.val_set, st.session_state.test_set = train_set, val_set, test_set
        ...
        # --- MODEL TRAINING ---
        if all([st.session_state.split_done, st.session_state.train_set is not None,
                st.session_state.val_set is not None, target_column, task_type, selected_models]):
